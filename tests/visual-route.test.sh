...
check_eq "svg prompt: interview before the model question" "$(ctx | head -n 1 | grep -c 'run the interview.*gaps --brief <brief.json>.*Then ask (one question, header "Model")')" "1"
...
check_eq "svg prompt: ask and generate.py carry --request-file" "$(ctx | grep -c -- '--request-file /')" "2"
...
check_eq "video prompt: names studio.py wait, for answers and for rounds" "$(ctx | grep -c 'studio.py wait --session')" "2"
...
check_eq "speech prompt: names studio.py wait, for answers and for rounds" "$(ctx | grep -c 'studio.py wait --session')" "2"
...
check_eq "svg and png prompt: one question set, two questions" "$(ctx | grep -c 'one question set holding 2 questions')" "1"
...
check_eq "svg and png prompt: names studio.py wait, for answers and for rounds" "$(ctx | grep -c 'studio.py wait --session')" "2"
...
check_eq "transparent PNG prompt: names studio.py wait, for answers and for rounds" "$(ctx | grep -c 'studio.py wait --session')" "2"
